...
export function Wheel({ prizes, config, isSpinning, onSpinEnd }: WheelProps) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 36; i++) {
      ...
      const dotX = centerX + (radius + 10) * Math.cos(angle)
      const dotY = centerY + (radius + 10) * Math.sin(angle)

      ctx.beginPath()
      ctx.arc(dotX, dotY, 3, 0, 2 * Math.PI)
      ctx.fillStyle = i % 2 === 0 ? config.borderColor || "#FF9FF3" : "#A5F1E9"
      ctx.fill()
    }
    ...
    prizes.forEach((prize, index) => {
      ...
      gradient.addColorStop(1, prize.backgroundColor || `hsl(${index * 60}, 70%, 50%)`)
